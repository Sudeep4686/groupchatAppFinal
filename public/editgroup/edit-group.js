...
const getGroups = async () =>{
    ...
    try{
        ...
        console.log("response in edit-group",response);
...
const getUsers=async()=>{
    ...
    try{
        const response = await axios.get(`${baseUrl}/groups/getMembers?gpId=${gpId}`,
        {
            headers:{Authorization:token},
        }
        );
...
const displayUsers= (user,isMember)=>{
    ...
    if(isMember){
        ...
        if(currentUser.id===user.id){
            button.appendChild(createTextNode("Exit Group"));
        }else{
...
async function removeAdminHandler(e){
    ...
    try{
        ...
        getUsers();
        console.log("Successfully removed as admin");
    }catch(error){
        console.log(error);
    }
}
