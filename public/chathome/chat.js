...
newGroup.addEventListener("click",()=>{
    window.location.href = "../newgroup/group.html";
});
...
const getGroups = async () => {
    try{
        ...
        console.log("Error while getting groups",err.message);
    }
};
...
const getChats = async () => {
    ...
    if (gpId) {
        ...
        console.log("gpmessages",gpMessages);
        console.log("lastmsgg",lastMsgId);
        try{
            const response = await axios.get(`${baseUrl}/chat/chats?lastMsgId=${lastMsgId}&gpId=${gpId}`,
            {
                headers:{ Authorization:token },
            }
            );
...
const getmembers = async () =>{
    ...
    try{
        ...
        console.log(response,"response in chat.js")
    ...
    if(file){
        ...
        reader.onload=()=>{
            ...
            socket.emit("upload",fileData,(fileUrl)=>{
                ...
                console.log("NAME IN FILEHANDLER",name);
            });
        };
