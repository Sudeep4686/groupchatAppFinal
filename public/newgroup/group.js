...
const submitHandler = async (e) => {
    ...
    if (groupName.value === "") {
      ...
      try {
        ...
        const gpId = response.data.group.id;
        const gpName = response.data.group.name;
...
const getUsers = async () => {
    ...
    try {
      ...
      console.log("getting users in group.js",users)
      users.forEach((user) => {
        displayUsers(user);
      });
    } catch (error) {
...
const addUserHandler = async (e) => {
    console.log("entering the adduser handler");
    const btn = e.target;
    const li = e.target.parentElement;
    const gpId = localStorage.getItem("newGroupId");
    console.log("checking gpID in frontend",gpId);
    const userId = li.id;
    try {
      await axios.get(
